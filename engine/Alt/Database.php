...
abstract class Alt_Database {
	...
	/**
	 * @var  array  Alt_Database instances
	 */
	public static $instances = array();

	/**
	 * Get a singleton Alt_Database instance. If configuration is not specified,
	 * it will be loaded from the database configuration file using the same
	 * group as the name.
	 *
	 *     // Load the default database
	 *     $db = Alt_Database::instance();
	 *
	 *     // Create a custom configured instance
	 *     $db = Alt_Database::instance('custom', $config);
	 *
	...
	public static function instance($name = NULL, array $config = NULL)
	{
		if ($name === NULL)
		{
			...
			$name = Alt_Database::$default;
		}

		if ( ! isset(Alt_Database::$instances[$name]))
		{
			if ($config === NULL)
			{
			...
			$driver = 'Alt_Database_'.ucfirst($config['type']);

			// Create the database connection instance
			$driver = new $driver($name, $config);

			// Store the database instance
			Alt_Database::$instances[$name] = $driver;
		}

		return Alt_Database::$instances[$name];
	}
	 ...
	 *
	 * [!!] This method cannot be accessed directly, you must use [Alt_Database::instance].
	 *
	 * @return  void
	 */
	 ...
	 *     unset(Alt_Database::instances[(string) $db], $db);
	 *
	 * [!!] Calling `unset($db)` is not enough to destroy the database, as it
	 * will still be stored in `Alt_Database::$instances`.
	 *
	 * @return  void
	 */
	...
	/**
	 * Disconnect from the database. This is called automatically by [Alt_Database::__destruct].
	 * Clears the database instance from [Alt_Database::$instances].
	 *
	 *     $db->disconnect();
	 *
	 * @return  boolean
	...
	{
		unset(Alt_Database::$instances[$this->_instance]);

		return TRUE;
	}

	/**
	 * Set the connection character set. This is called automatically by [Alt_Database::connect].
	 *
	 *     $db->set_charset('utf8');
	 *
	 * @throws  Alt_Exception
	 ...
	 *     $db->query(Alt_Database::SELECT, 'SELECT * FROM groups', TRUE);
	 *
	 *     // Make a SELECT query and use "Model_User" for the results
	 *     $db->query(Alt_Database::SELECT, 'SELECT * FROM users LIMIT 1', 'Model_User');
	 *
	 * @param   integer  $type       Alt_Database::SELECT, Alt_Database::INSERT, etc
	...
	public function count_records($table)
	{
		...
		return $this->query(Alt_Database::SELECT, 'SELECT COUNT(*) AS total_row_count FROM '.$table, FALSE)
			->get('total_row_count');
	}
	 ...
	 * @return  string
	 * @uses    Alt_Database::quote_identifier
	 * @uses    Alt_Database::table_prefix
	 */
